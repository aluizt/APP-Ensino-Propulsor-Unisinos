...
import { MatematicaArquiteturaPage } from '../matematica-arquitetura/matematica-arquitetura';

@IonicPage()
@Component({
  selector: 'page-matematica-arquiteturaprincipal',
  templateUrl: 'matematica-arquiteturaprincipal.html',  
  providers: [ScreenOrientation]
})
export class MatematicaArquiteturaprincipalPage {
  ...
  ionViewDidLoad() {
    console.log('ionViewDidLoad MatematicaArquiteturaprincipalPage');
    ...
    for(var i=0; i <this.monitoria.length; ++i){
      if(this.monitoria[i].descricao == "MatematicaArq" && this.monitoria[i].campus == 1){
   ...
   console.log("monitorias matematica para arquitetura");
  ...
  goToMatematicaArquitetura(){
    this.navCtrl.push(MatematicaArquiteturaPage);
  }
