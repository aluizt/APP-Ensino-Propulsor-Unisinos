...
import { CalculoIiPage } from '../calculo-ii/calculo-ii';

@IonicPage()
@Component({
  selector: 'page-calculo-iiprincipal',
  templateUrl: 'calculo-iiprincipal.html',
  providers: [ScreenOrientation]
})
export class CalculoIiprincipalPage {
  ...
  ionViewDidLoad() {
    console.log('ionViewDidLoad CalculoIIprincipalPage');
    ...
    for(var i=0; i <this.monitoria.length; ++i){
      if(this.monitoria[i].descricao == "CalculoII" && this.monitoria[i].campus == 1){
  ...
  goToCalculoII(){
    this.navCtrl.push(CalculoIiPage);
  }
}
