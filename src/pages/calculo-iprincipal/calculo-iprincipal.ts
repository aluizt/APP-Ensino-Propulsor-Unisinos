...
import { CalculoIPage } from '../calculo-i/calculo-i';

@IonicPage()
@Component({
  selector: 'page-calculo-iprincipal',
  templateUrl: 'calculo-iprincipal.html',
  providers: [ScreenOrientation]
})
export class CalculoIprincipalPage {
  ...
  ionViewDidLoad(){
    console.log('ionViewDidLoad CalculoIprincipalPage');
    ...
    for(var i=0; i <this.monitoria.length; ++i){
      if(this.monitoria[i].descricao == "CalculoI" && this.monitoria[i].campus == 1){
   ...
   console.log("monitorias fisica Eletricidade");
  ...
  goToCalculoI(){
    this.navCtrl.push(CalculoIPage);
  }
