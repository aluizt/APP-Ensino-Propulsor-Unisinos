...
import { FisicoquimicaPage } from '../fisicoquimica/fisicoquimica';

@IonicPage()
@Component({
  selector: 'page-fisicoquimicaprincipal',
  templateUrl: 'fisicoquimicaprincipal.html',
  providers: [ScreenOrientation]
})
export class FisicoquimicaprincipalPage {
  ...
  ionViewDidLoad() {
    console.log('ionViewDidLoad FisicoquimicaprincipalPage');
    ...
    for(var i=0; i <this.monitoria.length; ++i){
      if(this.monitoria[i].descricao == "FisicoQ" && this.monitoria[i].campus == 1){
   ...
   console.log("monitorias Fisico Quimico dos Eletrolitos");
  ...
  goToFisicoQ(){
    this.navCtrl.push(FisicoquimicaPage);
  }
}
