...
import { PoaAlgebraVetorialPage } from '../poa-algebra-vetorial/poa-algebra-vetorial';

@IonicPage()
@Component({
  selector: 'page-poa-algebra-vetorial-principal',
  templateUrl: 'poa-algebra-vetorial-principal.html',
  providers: [ScreenOrientation]
})
export class PoaAlgebraVetorialPrincipalPage {
  ...
  ionViewDidLoad() {
    console.log('ionViewDidLoad PoaAlgebraVetorialPrincipalPage');
    ...
    for(var i=0; i <this.monitoria.length; ++i){
      if(this.monitoria[i].descricao == "AlgebraV" && this.monitoria[i].campus == 2){
   ...
   console.log("monitorias Agebra vetorial Poa");
  ...
  goToAlgebraV(){
    this.navCtrl.push(PoaAlgebraVetorialPage);
  }
}
