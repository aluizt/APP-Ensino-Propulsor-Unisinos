...
export class AlgebraPage {
  private lista_algebra    = new Array<any>();// cria um array do tipo any
  ...
  ionViewDidLoad() {
    ...
    for (var h=0; h < this.lista_algebra.length; ++h) {
        if (this.lista_algebra[h].id == 5) {
           this.monitoriaalgebra=this.lista_algebra[h].monitorias;             
           break;
        }
    }
    
    this.auxiliar2=this.monitoriaalgebra;
    

    for (var j=0; j<this.auxiliar2.length; ++j){
         this.auxiliar=this.auxiliar2[j].campus;
         if(this.auxiliar == 1 && this.auxiliar2[j].descricao == "AlgebraV"){  // se o campus for são leopoldo
