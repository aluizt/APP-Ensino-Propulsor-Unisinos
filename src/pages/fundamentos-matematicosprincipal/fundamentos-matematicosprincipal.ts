...
import { FundamentosMatematicosPage } from '../fundamentos-matematicos/fundamentos-matematicos';

@IonicPage()
@Component({
  selector: 'page-fundamentos-matematicosprincipal',
  templateUrl: 'fundamentos-matematicosprincipal.html',
  providers: [ScreenOrientation]
})
export class FundamentosMatematicosprincipalPage {
  ...
  ionViewDidLoad() {
    console.log('ionViewDidLoad FundamentosMatematicosprincipalPage');
    ...
    for(var i=0; i <this.monitoria.length; ++i){
      if(this.monitoria[i].descricao == "Fundamentos" && this.monitoria[i].campus == 1){
   ...
   console.log("monitorias Fundamentos matematicos");
  ...
  goToFundamentosMatematicos(){
    this.navCtrl.push(FundamentosMatematicosPage);
  }
}
