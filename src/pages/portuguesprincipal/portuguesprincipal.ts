...
import { PortuguesPage } from '../portugues/portugues';

@IonicPage()
@Component({
  selector: 'page-portuguesprincipal',
  templateUrl: 'portuguesprincipal.html',
  providers: [ScreenOrientation]
})
export class PortuguesprincipalPage {
  ...
  ionViewDidLoad() {
    ...
    for(var i=0; i <this.monitoria.length; ++i){
      if(this.monitoria[i].descricao == "Portugues" && this.monitoria[i].campus == 1){
   ...
   console.log("monitorias Portugues");
  ...
  goToPortugues(){
    this.navCtrl.push(PortuguesPage);
  }
